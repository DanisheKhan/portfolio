...
export default function Hero() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full min-h-screen bg-[#080809] flex flex-col overflow-hidden select-none"
    >

      {/* ══════════ PHOTO — right half, full-height bleed ══════════ */}
      <div className="hero-photo-wrap absolute inset-y-0 right-0 w-[55%] hidden lg:block pointer-events-none">
        <img
          src="/danish.jpeg"
          alt="Danish Khan"
        ...
        {/* ── Blend layers (order matters) ── */}

        {/* 1. Left-to-transparent: blends into the dark text area */}
        ...
        {/* 4. Right edge fade: prevents hard crop */}
      ...
      {/* ══════════ CONTENT ══════════ */}
      <div className="relative z-10 flex flex-col flex-1 max-w-7xl mx-auto w-full px-6 md:px-12">

        {/* Top meta */}
        <div className="hero-fade flex items-center justify-between pt-32">
        ...
        <div className="flex-1 flex flex-col justify-center py-12">
          <div className="max-w-[60%] min-w-[320px] flex flex-col gap-0">
            ...
            <div className="hero-fade flex items-center gap-3 mt-7">
              <Link
                to="/projects"
                className="group inline-flex items-center gap-2 h-11 px-6 rounded-full bg-primary-accent text-[#080809] text-xs font-bold tracking-widest uppercase transition-all duration-300 hover:shadow-[0_0_36px_rgba(197,168,128,0.4)] hover:-translate-y-0.5"
              >
                View Work
      ...
      {/* ══════════ STAT RAIL ══════════ */}
      <div className="relative z-10 border-t border-white/[0.05] w-full">
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-2 md:grid-cols-4 divide-x divide-white/[0.05]">
          {stats.map((s, i) => (
            <div key={i} className="hero-stat flex flex-col items-center gap-1 py-5 px-6">
              <span className="text-lg md:text-xl font-bold font-clash-display text-white">
                {s.value}
              </span>
              <span className="text-[9px] font-mono tracking-[0.22em] text-white/25 uppercase">
                {s.label}
              </span>
            </div>
          ))}
